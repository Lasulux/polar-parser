                ...
                for filemember in zip_ref.namelist():
                    if filemember.startswith("training-session") and filemember.endswith(".json"):
                        ...
                        with zip_ref.open(filemember) as file:
                            ...
                            tqdm.write(f"Processed training session file: {filemember} for user: {username}")

        tqdm.write(f"Total training session files found: {len(self.training_JSON_files)}")
        tqdm.write(f"Total training summary entries: {len(self.training_summary)}")
        tqdm.write(f"Total heart rate samples collected: {len(self.training_hr_samples)}")
        tqdm.write("starting returning training hr df")
        folder_path = Path(
            matching_folders[0]
        )  # Use the first matching folder, should be updated to handle multiple folders!!!
    ...
    def parse_exercise_summary(self, exercises: list, username: str):
        ...
        for ex in tqdm(exercises, desc=f"Parsing exercises for {username}"):
            try:
                ...
                tqdm.write(f"Skipping invalid exercise entry: {e}")

        tqdm.write(f"Found {len(exercise_list)} exercises for user {username}. Appending to DataFrame.")
            ...
            try:
                for sample in ex.get("samples", {}).get("heartRate", []):
                    sample_time = datetime.fromisoformat(sample["dateTime"]) + timedelta(
                        minutes=ex.get("timezoneOffset", 0)
                    )
                ...
                tqdm.write(f"Missing heart rate value for timepoint {sample['dateTime']}")
        hr_df = pd.DataFrame(hr_samples)

        if not hr_df.empty:
            tqdm.write(
                f"Found {len(hr_df)} heart rate samples for user {username} in exercise {ex.get('sport', 'unknown')}. Appending to DataFrame."
            )
            self.training_hr_samples.append(hr_df)
            tqdm.write(f"Total heart rate samples collected after append: {len(self.training_hr_samples)}")
            self.training_hr_df = pd.concat([self.training_hr_df, hr_df], ignore_index=True)
            tqdm.write(f"Total heart rate DataFrame size after concat: {self.training_hr_df.shape}")
