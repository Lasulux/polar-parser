...
def process_polar_data(zip_data_directory, output_dir, save_format, start_date=None, end_date=None):
    ...
    print(f"End of training parser. Found {len(training_parser.training_JSON_files)} training session files.")
    ...
    print(f"End of activity parser. Found {len(activity_parser.activity_JSON_files)} activity files.")
    ...
    sleep_parser = SleepParser(folder_of_zip_files=zip_data_directory, zip_file_pattern='polar-user-data-export*',start_date=start_date, end_date=end_date)
    ...
    if save_format is not None:
        for user in tqdm(users, desc="Saving user data"):
            ...
            user_nightly_recovery_breathing_data = nightly_recovery_breathing_data[nightly_recovery_breathing_data['username'] == user]
